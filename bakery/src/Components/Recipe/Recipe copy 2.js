...
const Recipe = (props) => {
	...
	useEffect( () => {
		const tt = async () => {
			...
			setUnits(v_units);
			// console.log('After async: VVVVV', v_units, v_equipments, v_ingredients);
			// v_units.map((value)=> units.push(value))
			// v_equipments.map((value)=> equipments.push(value))
			// v_ingredients.map((value)=> ingredients.push(value))
	...
	useEffect ((()=>setRecipe(props.recipe)),[props.recipe])
